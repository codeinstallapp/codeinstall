...
// 【重要】
// 1. 调用 get 方法，如果 code 不为空字符串，我们即认为是【一次有效匹配安装】；如果 code 为空字符串，我们即认为是【一次有效未匹配安装】（这种情况一般非常低）。
// 2. 为了统计准确请自行做好缓存，避免重复统计安装，只要 get 方法正常返回（【即：无异常】。无论 code 是否为空字符），以后都不应该再调用 Codeinstallsdk 的任何方法。
// 3. 建议结合自身的用户账号系统，将获取到的 code（无论 code 是否为空字符） 和用户账号关联。比如：等用户用账号登录后，如果当前账号曾经获取过 code （无论曾经 code 是否为空字符），将不再获取。
